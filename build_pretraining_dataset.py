...
class ExampleWriter(object):
    ...
    def write_examples_hugging(self, f):
        for line in f:
            line = line.strip()
            if line or self._blanks_separate_docs:
                example = self._example_builder.add_line(line)
...
def write_examples(job_id, data, args):
    ...
    random.shuffle(data)
    start_time = time.time()
    for no, f in enumerate(data):
        if no > 0:
            elapsed = time.time() - start_time
            log("processed {:}/{:} files ({:.1f}%), ELAPSED: {:}s, ETA: {:}s, "
                "{:} examples written".format(
                no, len(data), 100.0 * no / len(data), int(elapsed),
                int((len(data) - no) / (no / elapsed)),
                example_writer.n_written))
        example_writer.write_examples_hugging(os.path.join(args.corpus_dir, f))
...
def main():
    ...
    parser.add_argument("--corpus-dir", required=False,
    ...
    bert_data = {}
    # [f for (i, f) in enumerate(fnames)
    #            if i % args.num_processes == job_id]
    for (i, f) in enumerate(bert_dataset):
        if bert_data[i % args.num_processes] is None:
            bert_data[i % args.num_processes] = []
        bert_data[i % args.num_processes].append(f)
    print("loaded bert datasets")
    ...
    if args.num_processes == 1:
        ...
        for i in range(args.num_processes):
            job = multiprocessing.Process(target=write_examples, args=(i, bert_data[i], args))
            jobs.append(job)
            job.start()
        for job in jobs:
            job.join()
